...
const LiquorRatings = ({liquorToDisplay, switchComp}) => {
    ...
    async function getUserRating(liquor) {
        try {
            const response = await axios.get(buildPath('api/userLiquorRating'), {
                params: { UserId: userID, _id: liquor._id }
            });
